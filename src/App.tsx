...
function App() {
  ...
  const [words, setWords] = useState(["PRIZ"]);
  const [word, setWord] = useState("PRIZ");
  ...
  const shuffleWord = useCallback((item : string) => {
    const shuffled = item.split('').sort(() => 0.5 - Math.random()).join('')
    if (shuffled === item || shuffled === word) {
      return shuffleWord(item); // Ensure it's different
    }
  ...
  const fetchWord = async () => {
    if (gameState === 'PLAYING') {
      ...
      const urlAddon = difficulty === 'EASY' ? `length=${getRandomInRange(4, 5)}` : difficulty === 'MEDIUM' ? `length=${getRandomInRange(6, 8)}` : `length=${getRandomInRange(9, 12)}`;
      setLoading(true);
      try {
        ...
        console.log(data)
        setWords(data);
        setWord(shuffleWord(data[0]));
      } catch (error) {
        console.error('Error fetching word:', error);
        setWords(["PRIZ"]); // Fallback word list
      } finally {
        setLoading(false);
      }
    }
  ...
  // next word function
  const loadNextWord = () => {
    const nextWord = words[index + 1] || "PRIZ"; // Fallback if words run out
    setWord(shuffleWord(nextWord));
  ...
  const skipWord = () => {
    setTimer(24);
    setInputValue("");
    setScore(prev => Math.max(prev - 50, 0)); // Penalty for skipping
  ...
  useEffect(() => {
    ...
    if (gameState === 'PLAYING') {
      interval = setInterval(() => {
        setTimer((prev) => {
          // If time reaches 0, reset to 24
          if (prev <= 1) {
            skipWord(); // Automatically skip the word
            return 24; 
          } if (prev <= 1 && wordsCount === 0) {
            setGameState('FINISHED');
            return 24;
          } else {
            return prev - 1;
  ...
  const handleSubmit = () => {
    if (inputValue === word) {
      ...
      setScore(prev => Math.max(prev - 50, 0)); // Penalty for wrong answer
    }
  }
      ...
      <main className="relative z-10 w-full flex items-center justify-center max-w-md">
        {gameState === 'START' ? (
          ...
          <div className="space-y-6 w-fit">
            ...
            {loading && <div className='w-full h-full absolute flex justify-center items-center left-0 top-0 z-[100]'><span>Loading...</span></div>}
            {/* Score & Timer HUD */}
            ...
            <div className={`bg-[#1A1D23] rounded-[2.5rem] p-8 shadow-2xl border border-white/5 ${loading ? 'opacity-50' : 'opacity-100'}`}>
              ...
              <div className="flex gap-2 mb-6">
                <button className="flex-1 py-3 bg-[#252932] hover:bg-[#2e333d] border border-white/5 rounded-xl text-[10px] font-black tracking-widest transition-all uppercase"
                  onClick={() => setWord(shuffleWord(word))}
                >
                  Shuffle
                </button>
              ...
              <div className="relative group">
                <input 
                  ...
                  className="w-full bg-transparent border-b-2 border-gray-800 py-4 text-center text-4xl font-black outline-none focus:border-teal-400 transition-colors tracking-widest placeholder:text-gray-800"
                  placeholder="----"
                  autoFocus
                />
